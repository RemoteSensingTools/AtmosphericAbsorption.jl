...
# Uniform per-line entry points (see profiles.jl). SDV is pCqSDHC with νVC=η=0.
@inline evaluate(::SpeedDependentVoigt, cpf::AbstractCPF, νI::FT, ν0::FT, p) where {FT} =
    real(pcqsdhc(cpf, ν0, p.γd, p.Γ0, p.Γ2, p.Δ0, p.Δ2, zero(FT), zero(FT), νI))
@inline evaluate(::HartmannTran, cpf::AbstractCPF, νI::FT, ν0::FT, p) where {FT} =
    real(pcqsdhc(cpf, ν0, p.γd, p.Γ0, p.Γ2, p.Δ0, p.Δ2, p.νVC, p.η, νI))
